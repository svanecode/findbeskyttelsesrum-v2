...
- If touching importer performance, does the change speed up the apply phase through batching, preloading, or lighter unchanged writes without weakening source coverage, lifecycle safety, or municipality canonicalization?
...
- If touching importer failure handling, do shelter and shelter-source write failures surface real table/status/error details plus safe identity context instead of stopping at a slug or source reference alone?
